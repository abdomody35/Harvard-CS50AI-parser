...
def preprocess(sentence):
    ...
    converted_words = []
    while initial_words:
        word = initial_words.pop(0)
        if any((letter >= 'a' and letter <= 'z') or (letter >= 'A' and letter <= 'Z') for letter in word):
...
def np_chunk(tree):
    ...
    return [chunk for chunk in tree.subtrees() if chunk.label() == "N"]
